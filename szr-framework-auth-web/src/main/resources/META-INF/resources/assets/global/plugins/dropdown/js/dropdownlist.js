...
var DropDownList = (function($,undefined) {
    ...
    DDLClass.prototype = {
        ...
        removeAll:function(){
        	this._removeAllListItem();
        	var len=this.select[0].options.length;
        	for(var i=0;i<len;i++){
        		this.select[0].options.remove(0);
        	}
        },
        add : function(option){
            var opt = new Option(option.text,option.value,option.selected?true:false);
